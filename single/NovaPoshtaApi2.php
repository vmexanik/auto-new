...
class NovaPoshtaApi2 {
	 ...
	 * @return array City's data
	 */
	public function getCities2($areaName) {
	    // Get all cities 
	 ...
	 * @return mixed
	 */
	function getCounterparties($counterpartyProperty = 'Recipient', $page = NULL, $findByString = NULL, $cityRef = NULL, $edrpou = NULL) {
		...
		$edrpou AND $params['EDRPOU'] = $edrpou;
	...
	function getStatusDocuments($aDocument) {
	...
	function newInternetDocument($sender, $recipient, $params) {
		...
		if ( ! $recipient['Recipient']) {
			...
			if($params['ServiceType']=='WarehouseDoors'){
			    $options["CounterpartyRef"]=$recipient['Recipient'];
			    $options["StreetRef"]=$recipient['street'];
			    $options["BuildingNumber"]=$recipient['house'];
			    $options["Flat"]=$recipient['apartment'];
			    $recipientAdress = $this->model('Address')->save($options);
			    if($recipientAdress){
			        $recipient['RecipientAddress']=$recipientAdress['data'][0]['Ref'];
			    }
			}
		...
		$debug=serialize($paramsInternetDocument);
