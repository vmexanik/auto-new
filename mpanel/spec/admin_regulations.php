...
class AAdminRegulations extends Admin
{
	...
	public function UpdateCat($sCode) {
		...
		$aData = Db::getAll("Select id,id_tof,pref from cat where id_tof!=0");
	    ...
	    if (defined('REMOTE_TECDOC')) {
	    	...
	    	if ($aData) {
	    	    foreach ($aData as $aValue) {
	    	        $aTecDoc = TecdocDb::getAssoc("Select s2.ID_src as key_, s2.ID_src
	                from ".DB_OCAT."cat_alt_suppliers s
	                inner join ".DB_OCAT."cat_alt_suppliers s2 on s2.Search = s.Search and s.ID_src=".$aValue['id_tof']."
	                where s2.ID_src!=".$aValue['id_tof']."
	                group by s2.ID_src");
	    	        if ($aTecDoc)
	    	            Db::Execute("Update cat set id_sync='".implode(",",$aTecDoc)."' where id=".$aValue['id']);
	    	    }
	    	}
	    }
	    else {
	    	...
	    	if ($aData) {
	    	    foreach ($aData as $aValue) {
	    	        $aTecDoc = Db::getAssoc("Select s2.ID_src as key_, s2.ID_src
	                from ".DB_OCAT."cat_alt_suppliers s
	                inner join ".DB_OCAT."cat_alt_suppliers s2 on s2.Search = s.Search and s.ID_src=".$aValue['id_tof']."
	                where s2.ID_src!=".$aValue['id_tof']."
	                group by s2.ID_src");
