    ...
    public function Index() {
        ...
        if ($this->aSearch) {
            if (Language::getConstant('mpanel_search_strong',0)) {
                if ($this->aSearch['id'])$this->sSearchSQL .= " and a.id = '".$this->aSearch['id']."'";
                if ($this->aSearch['login'])	$this->sSearchSQL .= " and a.login = '".$this->aSearch['login']."'";
                if ($this->aSearch['last_login'])	$this->sSearchSQL .= " and a.last_login = '".$this->aSearch['last_login']."'";
                if ($this->aSearch['last_referer'])	$this->sSearchSQL .= " and a.last_referer = '".$this->aSearch['last_referer']."'";
                if ($this->aSearch['type_'])	$this->sSearchSQL .= " and a.type_ = '".$this->aSearch['type_']."'";
            }
            else {
                if ($this->aSearch['id'])$this->sSearchSQL .= " and a.id like '%".$this->aSearch['id']."%'";
                if ($this->aSearch['login'])	$this->sSearchSQL .= " and a.login like '%".$this->aSearch['login']."%'";
                if ($this->aSearch['last_login'])	$this->sSearchSQL .= " and a.last_login like '%".$this->aSearch['last_login']."%'";
                if ($this->aSearch['last_referer'])	$this->sSearchSQL .= " and a.last_referer like '%".$this->aSearch['last_referer']."%'";
                if ($this->aSearch['type_'])	$this->sSearchSQL .= " and a.type_ like '%".$this->aSearch['type_']."%'";
            }
        }
