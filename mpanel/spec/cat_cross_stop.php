 ...
 *
 */
class ACatCrossStop extends Admin {
	//-----------------------------------------------------------------------------------------------
	function __construct() {
		$this->sTableName='cat_cross_stop';
		$this->sTablePrefix='cc';
		$this->sTableId='id';
		$this->sAction='cat_cross_stop';
		$this->sSqlPath = "Catalog/PartCrossStop";
		$this->sWinHead=Language::getDMessage('cross_stop');
	...
	public function AfterApply($aBeforeRow,$aAfterRow) {
	    if($aBeforeRow) {
	        Db::Execute("delete from cat_cross_stop where pref='".$aBeforeRow['pref_crs']."' and code='".$aBeforeRow['code_crs']."' and pref_crs='".$aBeforeRow['pref']."' and code_crs='".$aBeforeRow['code']."' ");
	    }
	    
	    Db::Execute(" insert ignore into cat_cross_stop (pref, code, pref_crs, code_crs, source) values 
	        ('".$aAfterRow['pref_crs']."','".$aAfterRow['code_crs']."','".$aAfterRow['pref']."','".$aAfterRow['code']."','".$aAfterRow['source']."' )");
	}
	//-----------------------------------------------------------------------------------------------
	    ...
	    if (is_array ( Base::$aRequest ['row_check'] )) {
	        ...
	        if($aDelTmp) {
	            foreach ($aDelTmp as $aDelRow) {
	                Db::Execute("delete from cat_cross_stop where pref='".$aDelRow['pref_crs']."' and code='".$aDelRow['code_crs']."' and pref_crs='".$aDelRow['pref']."' and code_crs='".$aDelRow['code']."' ");
	            }
	        }
	...
	public function  Import(){
	    $this->sAction = "cat_cross_stop/import";
