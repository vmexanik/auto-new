...
{/if}
{if $sAfterTableHeader}
	<div class="after_header">{$sAfterTableHeader}</div>
{/if}
...
{if $bCheckVisible && $aItem}
<tr>
	<td colspan="20"><label for="check_all" class="for_check_label"><input type="checkbox" class="check_all js-checkbox" id="check_all" name="check_all"><span class="text">{$oLanguage->getMessage('select_all')}</span></label></td>
</tr>
{/if}
...
<tr {if $aItem[d].tr_add_text}{$aItem[d].tr_add_text}{/if} id="{$sIdiTr}{$iTr}" {if $bHideTr} pn="{$aItem[d].iHideTr}"{/if}
