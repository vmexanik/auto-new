...
<!--ul class="secodary_tabs">
	<li class="{if $aTemplateParameter=='profile'}sel{/if}"
		...
		>{$oLanguage->GetMessage('Csutomer Profile')}</a></li>
	...
	<li class="{if $aTemplateParameter=='subcustomer'}sel{/if}"
		><a href='/?action=customer_subcustomer'
		>{$oLanguage->GetMessage('Sub castomers')}</a></li>
