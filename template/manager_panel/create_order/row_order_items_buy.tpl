...
<div class="col-lg-2">
    <button class="btn btn-sm btn-success" type="button"
    	onclick="xajax_process_browse_url('/?action=manager_panel_create_order_add_find_apply&id={$aRow.id}&number='+ encodeURIComponent($('#number_cart_{$aRow.id}').val()))">{$oLanguage->getMessage('in_order')}</button>
</div>
</td>
