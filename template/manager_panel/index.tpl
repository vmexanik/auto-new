...
{foreach key=key item=aValue from=$aColumn}
	...
	{if !$aValue.nosort}
		{if $key==$sTablePriceSort && $sTablePriceSortWay == 'up'}
			<div class="th-inner sortable both asc">
 				<a class="common-th" href="{$sSeoUrl}&{$sType}sort={$sSort}&{$sType}way=down"
		...
		{elseif $key==$sTablePriceSort && $sTablePriceSortWay == 'down'}
			<div class="th-inner sortable both desc">
				<a class="common-th" href="{$sSeoUrl}&{$sType}sort={$sSort}&{$sType}way=up"
				onclick=" xajax_process_browse_url(this.href); return false;">
		{else}
	...
	{else}
		<div class="th-inner">{$aValue.sTitle}{if !$aValue.sTitle}&nbsp;{/if}</div>
	{/if}
...
<tr id="{$sIdiTr}{$iTr}" {if $bHideTr} pn="{$aItem[d].iHideTr}"{/if}
	{if $aItem[d].tr_tooltip}
		{foreach key=Tkey item=TValue from=$aItem[d].tr_tooltip}
			{$Tkey}="{$TValue}"
		{/foreach}
	{/if}
	...
	{if $bCheckVisible}<td><div class="form-check">{if !$aRow.bCheckHide}<label><input type=checkbox name=row_check[] class="form-check-input"
	...
	{if $sCheckAction!=''}onchange="{$sCheckAction}"{/if}>{/if}</label></div></td>
	{/if}
